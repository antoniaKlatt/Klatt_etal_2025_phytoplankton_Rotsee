...
d2H_cholOAc_stigmaOAc <- read.csv ("C:\\Users\\KlattA\\OneDrive - Universität Basel\\Desktop\\Uni Basel\\Basel AG Ladd\\PhD project\\Rotsee\\Rotsee2019 Water column\\Correlations\\Files for R\\files_Rotsee_watercolumn_1920\\d2Hm_d2Hreal_cholOAc_stigmaOAc.csv", header = TRUE , sep = ";" , stringsAsFactors = FALSE, na.strings = "#NV")
...
pd2H_peakArea <- ggplot() + 
  ...
  annotate("text",y= -165, x= -40,label="A", size=15, fontface=2, color="black")+
  ...
  theme(panel.background = element_blank(),
        ...
        plot.margin = unit(c(1.5,0.25,0.25,0.5), "cm"))
...
pEps_mr_peakArea <- ggplot() + 
  ...
  scale_x_continuous(breaks=c(0,40,80,120))+
  annotate("text",y= 135, x= -35,label="B", size=15, fontface=2, color="black")+
  annotate("text",y= 40, x= 80,label="y = -23.21 x ln(x) + 103", size=10, fontface=1, color="black")+
  ylab(expression(epsilon^{2}*""['measured/calibrated']~("\211"))) + xlab("peak area (Vs)")+
  theme(panel.background = element_blank(),
        plot.title=element_text(size=35, color= 'black', face= 'bold'),
        plot.subtitle=element_text(size=15, color= 'white'),
        legend.position = "right",
...
FigS1 <- plot_grid(pd2H_peakArea, pEps_mr_peakArea , ncol=2,rel_widths = c(0.75,1), align="h")
