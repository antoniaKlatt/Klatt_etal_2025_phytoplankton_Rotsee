...
phyto_zoo_bioVolume <- read.csv ("C:\\Users\\KlattA\\OneDrive - Universität Basel\\Desktop\\Uni Basel\\Basel AG Ladd\\PhD project\\PhD thesis\\Chapter 1 ROT19\\Supplementary_C1\\Tables_C1\\phyto_microzooplankton_biovolume_ROT19.csv", header = TRUE , sep = ";" , stringsAsFactors = FALSE, na.strings = "#NV")
...
pbioVol_surface <-ggplot()+
  ...
  geom_area(data=Zoo_surface, aes(x = date, y=value, fill= variable), alpha=0.275)+
  geom_line(data=Phyto_surface, aes(x = date, y=value, color= variable),linewidth=1.175)+
  ...
  scale_fill_manual(labels= c(Gymnodinium_helveticum_um3L= expression(italic("Gymnodinium helveticum")), 
                    ...
                    values=c(Gymnodinium_helveticum_um3L= "orchid", Ciliata_um3L= "lightskyblue3", Rotifera_um3L= "yellow3"),
  ...
  scale_color_manual(labels=c(cyanobacteria_um3L=expression(italic("Cyanophyceae")), 
                     ...
                     values= c(cyanobacteria_um3L="cyan4", green.algae_um3L="green3", Chrysophyta_um3L="darkgoldenrod1",
                               Diatoms_um3L= "gray65" , Dinoflagellates_um3L= "salmon", Cryptophyceae_um3L= "maroon4"),
                     name= "phytoplankton",
...
plegend <- get_legend(pbioVol_surface+ theme(legend.title = element_text(size=30, face= "bold"),
                             ...
                             guides(colour = guide_legend(title.position="top", override.aes = list(linewidth=3), nrow=2),
...
pbioVol_chlorMax <- ggplot()+
  ...
  geom_area(data=zooBio_chlorMax, aes(x = date, y=Gymnodinium_helveticum_um3L), fill="orchid" , alpha=0.275)+
  geom_area(data=zooBio_chlorMax, aes(x = date, y=Ciliata_um3L), fill="lightskyblue3" , alpha=0.275)+
  geom_area(data=zooBio_chlorMax, aes(x = date, y=Rotifera_um3L), fill="yellow3" , alpha=0.275)+  
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=cyanobacteria_um3L),color="cyan4", linewidth=1.175)+
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=green.algae_um3L),color="green3", linewidth=1.175)+
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=Chrysophyta_um3L),color="darkgoldenrod1", linewidth=1.175)+
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=Diatoms_um3L),color="gray65", linewidth=1.175)+
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=Dinoflagellates_um3L),color="salmon", linewidth=1.175)+
  geom_line(data=phytoBio_chlorMax, aes(x = date, y=Cryptophyceae_um3L),color="maroon4", linewidth=1.175)+
